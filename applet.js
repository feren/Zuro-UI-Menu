...
const AppletPath = imports.ui.appletManager.applets['zuromenu@feren'];
...
App.prototype = {
    ...
    _fromCommand: function(command) {
        ...
        this.icon = new Gio.ThemedIcon({ name: 'system-run-symbolic' });
...
AppEntry.prototype = {
    ...
    _init: function(app) {
        ...
        this.iconSize = zuromenu.iconSize;
        this.icon = app.icon;
...
function zuromenuView(launcher, orientation) {
    this._init(launcher, orientation);
}

zuromenuView.prototype = {
    ...
    _init: function(launcher, orientation) {
        ...
        if (zuromenu.screenResolution != resolution)
            this._setupSize();
    ...
    _setupSize: function() {
        ...
        zuromenu.screenResolution = Main.layoutManager.primaryMonitor.width + 'x' + Main.layoutManager.primaryMonitor.height;
        ...
        if (zuromenu.columns != this._defaultColumns)
            zuromenu.columns = this._defaultColumns;
        if (zuromenu.rows != this._defaultRows)
            zuromenu.rows = this._defaultRows;
    ...
    _setupUi: function() {
        ...
        this.searchbar.actor.set_width(500);
        ...
        let btnIcon = new St.Icon({
            icon_size: 20,
            gicon: new Gio.ThemedIcon({ name: 'gnome-session-logout' })
        });
    ...
    _readSettings: function(firstStart, checkColumns, checkRows) {
        ...
        if (checkColumns) {
            if (zuromenu.columns > 3)
                this._defaultColumns = zuromenu.columns;
            else
                this._defaultColumns = zuromenu.columns = 4;
            global.log("Cols: " + zuromenu.columns);
        }

        if (checkRows) {
            if (zuromenu.rows > 1)
                this._defaultRows = zuromenu.rows;
            else
                this._defaultRows = zuromenu.rows = 2;
...
Signals.addSignalMethods(zuromenuView.prototype);
...
var zuromenu = {
    ...
    _init: function(orientation, panelHeight, instanceId) {
        ...
        try {
            this.settings = new Settings.AppletSettings(this, 'zuromenu@feren', instanceId);

            this.settings.bindProperty(Settings.BindingDirection.BIDIRECTIONAL, 'zuromenu-icon-size', 'iconSize');
            this.settings.bindProperty(Settings.BindingDirection.BIDIRECTIONAL, 'zuromenu-columns', 'columns', Lang.bind(this, function() {
                this._view._readSettings(false, true, false);
            }));
            this.settings.bindProperty(Settings.BindingDirection.BIDIRECTIONAL, 'zuromenu-rows', 'rows', Lang.bind(this, function() {
                this._view._readSettings(false, false, true);
            }));
            this.settings.bindProperty(Settings.BindingDirection.BIDIRECTIONAL, 'zuromenu-use-category', 'useCategory');
            this.settings.bindProperty(Settings.BindingDirection.BIDIRECTIONAL, 'zuromenu-screen-resolution', 'screenResolution');
            ...
            this._view = new zuromenuView(this, orientation);
            this._view.box.add_style_class_name('zuromenu');
...
function main(metadata, orientation, panelHeight, instanceId) {
    zuromenu._init(orientation, panelHeight, instanceId);
    return zuromenu;
}
